...
buff[0]=1;
sendto(sock, &buff, 1, 0, (struct sockaddr *) &bcastAddr, addrLen); // send a peer start announcement to broadcast address
...
for(;;) {
        ...
        if(FD_ISSET(0,&rfds)) // user wrote something on the console
			...
			for(i=0;i<MAX_PEERS;i++)
                      	   if(peerActive[i])
				{
				getnameinfo((struct sockaddr *) &peerAddr[i], addrLen, buff,BUF_SIZE,NULL,0,NI_NUMERICHOST|NI_NUMERICSERV);
				printf(" %s",buff);
				}
			printf("\n");
			}
