...
for(i=0; i<SERVER_NUM_PORTS; i++) 
  ...
  if(sock[i]==-1) {
        perror("Failed to open socket"); freeaddrinfo(list); i--; do {close(sock[i]); i--; } while(i>-1); exit(1);}

  if(bind(sock[i],(struct sockaddr *)list->ai_addr, list->ai_addrlen)==-1) {
        perror("Bind failed");freeaddrinfo(list);do {close(sock[i]); i--; } while(i>-1); exit(1);}
